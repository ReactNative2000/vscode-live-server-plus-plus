"use strict";
var __importStar = (this && this.__importStar) || function (mod) {
    if (mod && mod.__esModule) return mod;
    var result = {};
    if (mod != null) for (var k in mod) if (Object.hasOwnProperty.call(mod, k)) result[k] = mod[k];
    result["default"] = mod;
    return result;
};
Object.defineProperty(exports, "__esModule", { value: true });
const vscode = __importStar(require("vscode"));
const http = __importStar(require("http"));
const WebSocket = __importStar(require("ws"));
const path = __importStar(require("path"));
const FileSystem_1 = require("./FileSystem");
const utils_1 = require("./utils");
const LSPPError_1 = require("./LSPPError");
const urlJoin_1 = require("../extension/utils/urlJoin");
class LiveServerPlusPlus {
    constructor(config) {
        this.middlewares = [];
        this.wsWatcherList = [];
        this.init(config);
        this.goLiveEvent = new vscode.EventEmitter();
        this.goOfflineEvent = new vscode.EventEmitter();
        this.serverErrorEvent = new vscode.EventEmitter();
    }
    get onDidGoLive() {
        return this.goLiveEvent.event;
    }
    get onDidGoOffline() {
        return this.goOfflineEvent.event;
    }
    get onServerError() {
        return this.serverErrorEvent.event;
    }
    get isServerRunning() {
        return this.server ? this.server.listening : false;
    }
    reloadConfig(config) {
        this.init(config);
    }
    async goLive() {
        if (this.isServerRunning) {
            return this.serverErrorEvent.fire({
                LSPP: this,
                code: 'serverIsAlreadyRunning',
                message: 'Server is already running'
            });
        }
        try {
            await this.listenServer();
            this.registerOnChangeReload();
            this.goLiveEvent.fire({ LSPP: this });
        }
        catch (error) {
            if (error.code === 'EADDRINUSE') {
                return this.serverErrorEvent.fire({
                    LSPP: this,
                    code: 'portAlreadyInUse',
                    message: `${this.port} is already in use!`
                });
            }
            return this.serverErrorEvent.fire({
                LSPP: this,
                code: error.code,
                message: error.message
            });
        }
    }
    async shutdown() {
        if (!this.isServerRunning) {
            return this.serverErrorEvent.fire({
                LSPP: this,
                code: 'serverIsNotRunning',
                message: 'Server is not running'
            });
        }
        await this.closeWs();
        await this.closeServer();
        this.goOfflineEvent.fire({ LSPP: this });
    }
    useMiddleware(...fns) {
        fns.forEach(fn => this.middlewares.push(fn));
    }
    useService(...fns) {
        fns.forEach(fn => {
            const instance = new fn(this);
            instance.register.call(instance);
        });
    }
    init(config) {
        this.cwd = config.cwd;
        this.indexFile = config.indexFile || 'index.html';
        this.port = config.port || 9000;
        this.debounceTimeout = config.debounceTimeout || 400;
        this.reloadingStrategy = config.reloadingStrategy || 'hot';
    }
    registerOnChangeReload() {
        let timeout;
        vscode.workspace.onDidChangeTextDocument(event => {
            //debouncing
            clearTimeout(timeout);
            timeout = setTimeout(() => {
                const fileName = event.document.fileName;
                const action = this.getReloadingActionType(fileName);
                const filePathFromRoot = urlJoin_1.urlJoin(fileName.replace(this.cwd, '')); // bit tricky. This will change Windows's \ to /
                this.broadcastWs({
                    dom: ['hot', 'partial-reload'].indexOf(action) !== -1
                        ? event.document.getText()
                        : undefined,
                    fileName: filePathFromRoot
                }, action);
            }, this.debounceTimeout);
        });
    }
    getReloadingActionType(fileName) {
        const extName = path.extname(fileName);
        const isCSS = extName === '.css';
        const isInjectable = utils_1.isInjectableFile(fileName);
        if (isCSS) {
            return 'refreshcss';
        }
        if (isInjectable) {
            return this.reloadingStrategy;
        }
        return 'reload';
    }
    listenServer() {
        return new Promise((resolve, reject) => {
            if (!this.cwd) {
                const error = new LSPPError_1.LSPPError('CWD is not defined', 'cwdUndefined');
                return reject(error);
            }
            this.server = http.createServer(this.routesHandler.bind(this));
            const onPortError = reject;
            this.server.on('error', onPortError);
            this.attachWSListeners();
            this.server.listen(this.port, () => {
                this.server.removeListener('error', onPortError);
                resolve();
            });
        });
    }
    closeServer() {
        return new Promise((resolve, reject) => {
            this.server.close(err => {
                return err ? reject(err) : resolve();
            });
            this.server.emit('close');
        });
    }
    closeWs() {
        return new Promise((resolve, reject) => {
            if (!this.ws)
                return resolve();
            this.ws.close(err => (err ? reject(err) : resolve()));
        });
    }
    broadcastWs(data, action = 'reload') {
        if (!this.ws)
            return;
        let clients = this.ws.clients;
        //TODO: WE SHOULD WATCH ALL FILE. FOR NOW, THE LIB WORKS ONLY FOR HTML
        if (utils_1.isInjectableFile(data.fileName)) {
            clients = this.wsWatcherList.reduce((allClients, { client, watchingPaths }) => {
                if (this.isInWatchingList(data.fileName, watchingPaths))
                    allClients.push(client);
                return allClients;
            }, []);
        }
        clients.forEach(client => {
            if (client.readyState === WebSocket.OPEN) {
                client.send(JSON.stringify({ data, action }));
            }
        });
    }
    isInWatchingList(target, dirList) {
        for (let i = 0; i < dirList.length; i++) {
            let dir = dirList[i];
            //TODO: THIS IS NOT THE BEST WAY. IF FOLDER CONTANTS `.`, this will not work
            if (!path.extname(dir)) {
                dir = urlJoin_1.urlJoin(dir, this.indexFile);
            }
            if (target.startsWith('/'))
                target = target.substr(1);
            if (dir.startsWith('/'))
                dir = dir.substr(1);
            if (dir === target) {
                return true;
            }
        }
        return false;
    }
    attachWSListeners() {
        if (!this.server)
            throw new Error('Server is not defined');
        this.ws = new WebSocket.Server({ noServer: true });
        this.ws.on('connection', ws => {
            ws.send(JSON.stringify({ action: 'connected' }));
            ws.on('message', (_data) => {
                const { watchList } = JSON.parse(_data);
                if (watchList) {
                    this.addToWsWatcherList(ws, watchList);
                }
            });
            ws.on('close', () => this.removeFromWsWatcherList(ws));
        });
        this.ws.on('close', () => {
            console.log('disconnected');
        });
        this.server.on('upgrade', (request, socket, head) => {
            if (request.url === '/_ws_lspp') {
                this.ws.handleUpgrade(request, socket, head, ws => {
                    this.ws.emit('connection', ws, request);
                });
            }
            else {
                socket.destroy();
            }
        });
    }
    removeFromWsWatcherList(client) {
        const index = this.wsWatcherList.findIndex(e => e.client === client);
        if (index !== -1) {
            this.wsWatcherList.splice(index, 1);
        }
    }
    addToWsWatcherList(client, watchDirs) {
        const _watchDirs = Array.isArray(watchDirs) ? watchDirs : [watchDirs];
        this.wsWatcherList.push({ client, watchingPaths: _watchDirs });
    }
    applyMiddlware(req, res) {
        this.middlewares.forEach(middleware => {
            middleware(req, res);
        });
    }
    routesHandler(req, res) {
        const cwd = this.cwd;
        if (!cwd)
            return res.end('Root Path is missing');
        this.applyMiddlware(req, res);
        const file = req.file; //file comes from one of middlware
        const filePath = path.isAbsolute(file) ? file : path.join(cwd, file);
        const contentType = req.contentType || '';
        const fileStream = FileSystem_1.readFileStream(filePath, contentType.indexOf('image') !== -1 ? undefined : 'utf8');
        fileStream.on('open', () => {
            // TOOD: MAY BE, WE SHOULD INJECT IT INSIDE <head> TAG (although browser are not smart enought)
            if (utils_1.isInjectableFile(filePath))
                res.write(utils_1.INJECTED_TEXT);
            fileStream.pipe(res);
        });
        fileStream.on('error', err => {
            console.error('ERROR ', err);
            const _err = err;
            const errno = _err && _err.code;
            res.statusCode = errno === 'ENOENT' ? 404 : 500;
            return res.end();
        });
    }
}
exports.LiveServerPlusPlus = LiveServerPlusPlus;
